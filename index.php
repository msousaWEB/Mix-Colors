<!-- ======= Head ======= -->
<?php include_once("views/partials/head.php")?>

    <!-- ======= Navbar Section ======= -->
    <?php include_once("views/partials/layouts/navbar_section.php") ?>

    <!-- ======= Hero Section ======= -->
    <?php include_once("views/partials/layouts/hero_section.php")?>

  <main id="main">

    <!-- ======= About Section ======= -->
    <?php include_once("views/partials/layouts/about_section.php")?>

    <!-- ======= Services Section ======= -->
    <?php include_once("views/partials/layouts/services_section.php")?>

    <!-- ======= Why Us Section ======= -->
    <!-- <?php include_once("views/partials/layouts/whyus_section.php")?> -->

    <!-- ======= Call To Action Section ======= -->
    <?php include_once("views/partials/layouts/calltoaction_section.php")?>

    <!-- ======= Features Section ======= -->
    <!-- <?php include_once("views/partials/layouts/features_section.php")?> -->
  
    <!-- ======= Portfolio Section ======= -->
    <?php include_once("views/partials/layouts/portfolio_section.php")?>

    <!-- ======= Contact Section ======= -->
    <?php include_once("views/partials/layouts/contact_section.php")?>

  </main>

  <!-- ======= Footer ======= -->
<?php include_once("views/partials/footer.php") ?>
